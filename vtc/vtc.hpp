 ...
 */
template<typename T>
concept ValidIoVariableValueType =
    std::disjunction_v<std::is_same<T, Bit>, std::is_same<T, Byte>, std::is_same<T, Word>, std::is_same<T, Cardinal>,
 ...
 */
template<Byte Address, Byte FrameID, size_t FrameByteSize, io::IoBinding Binding, typename... FrameElementTs>
...
{
    using type =
        FrameBit<io::VehicleDetCall<Cabinet, Binding, N>,
...
{
    using type = typename CallDataFrameBitGenerator<Cabinet, Binding, N - 1,
                                                    typename CallDataFrameBit<Cabinet, Binding, N>::type, Ts...>::type;
};
...
using CallDataFrameBits = typename CallDataFrameBitGenerator<Cabinet, Binding, MAX_VEHICLE_DETECTORS>::type;
...
{
    using type = FrameBit<io::ChannelGreenWalkDriver<Cabinet, Binding, N>, 26 + 3 * (N - 1)>;
};
...
struct GreenChannelDriverFrameBitGenerator
{
    using type = typename GreenChannelDriverFrameBitGenerator<
        Cabinet, Binding, N - 1, typename GreenChannelDriverFrameBit<Cabinet, Binding, N>::type, Ts...>::type;
};
 ...
 */
template<CabinetIndex Cabinet, io::IoBinding Binding>
using GreenChannelDriverFrameBits =
    typename GreenChannelDriverFrameBitGenerator<Cabinet, Binding, MAX_LOAD_SWITCHES>::type;
...
{
    using type = FrameBit<io::ChannelYellowPedClearDriver<Cabinet, Binding, N>, 25 + 3 * (N - 1)>;
};
...
struct YellowChannelDriverFrameBitGenerator
{
    using type = typename YellowChannelDriverFrameBitGenerator<
        Cabinet, Binding, N - 1, typename YellowChannelDriverFrameBit<Cabinet, Binding, N>::type, Ts...>::type;
};
 ...
 */
template<CabinetIndex Cabinet, io::IoBinding Binding>
using YellowChannelDriverFrameBits =
    typename YellowChannelDriverFrameBitGenerator<Cabinet, Binding, MAX_LOAD_SWITCHES>::type;
...
{
    using type = FrameBit<io::ChannelRedDoNotWalkDriver<Cabinet, Binding, N>, 24 + 3 * (N - 1)>;
};
...
struct RedChannelDriverFrameBitGenerator
{
    using type = typename RedChannelDriverFrameBitGenerator<
        Cabinet, Binding, N - 1, typename RedChannelDriverFrameBit<Cabinet, Binding, N>::type, Ts...>::type;
};
 ...
 */
template<CabinetIndex Cabinet, io::IoBinding Binding>
using RedChannelDriverFrameBits = typename RedChannelDriverFrameBitGenerator<Cabinet, Binding, MAX_LOAD_SWITCHES>::type;
...
using SimulatorInitialHandshakeFrame =
    detail::Frame<254,// Simulator Address = 254
                  64, // FrameID = 64
...
template<CabinetIndex Cabinet, io::IoBinding Binding>
    requires ValidCabinetIndex<Cabinet>
using SimulatorInitialHandshakeAckFrame = detail::Frame<254,// Simulator Address = 254
                                                        192,// FrameID = 192
...
template<CabinetIndex Cabinet, io::IoBinding Binding>
    requires ValidCabinetIndex<Cabinet>
using SimulatorCallRequestFrame = detail::Frame<254,// Simulator Address = 254
                                                65, // FrameID = 65
                                                3,  // Total number of bytes of the instance
                                                Binding
                                                // ----------------------------------------------
...
template<CabinetIndex Cabinet, io::IoBinding Binding>
    requires ValidCabinetIndex<Cabinet>
using SimulatorLoadSwitchDriversAckFrame = detail::Frame<254,// Simulator Address = 254
                                                         194,// FrameID = 194
                                                         3,  // Total number of bytes of the instance
                                                         Binding
                                                         // ----------------------------------------------
...
template<typename T>
concept BiuType = requires { T::device_kind() == DeviceKind::Biu; };
...
private:
    using SilsBiuFrameMaps = frame::detail::FrameMaps<
        frame::detail::FrameMap<typename BiuFrameType<Cabinet, 64>::type, typename BiuFrameType<Cabinet, 192>::type>,
        frame::detail::FrameMap<typename BiuFrameType<Cabinet, 65>::type, typename BiuFrameType<Cabinet, 193>::type>,
        frame::detail::FrameMap<typename BiuFrameType<Cabinet, 66>::type, typename BiuFrameType<Cabinet, 194>::type>>;

    const SilsBiuFrameMaps frame_maps_{
        std::make_tuple(std::make_tuple(std::ref(frame::Global::instance<typename BiuFrameType<Cabinet, 64>::type>),
...
using LoadSwitchDriver /**/
    = std::tuple<io::ChannelGreenWalkDriver<Cabinet, IoBinding::Fio, Ch> &,
...
{
    return std::make_tuple(std::ref(Global::instance<ChannelGreenWalkDriver<Cabinet, IoBinding::Fio, Ch>>),      /**/
...
template<CabinetIndex Cabinet>
using LoadSwitchWirings = decltype(make_wirings<Cabinet>(LoadSwitchWiringFactory{}, LoadSwitchChannels{}));
...
private:
    io::VehicleDetCall<Cabinet, io::IoBinding::Fio, Ch> &state_{
        io::Global::instance<io::VehicleDetCall<Cabinet, io::IoBinding::Fio, Ch>>};
};
...
struct ISupportXinLoopSimulation
{
   ...
   */
    virtual void SetSignalState(SignalGroupID sg_id, ApproachID approach_id, TurnIndex turn_id,
...
template<typename T>
concept RackType = requires { T::device_kind() == DeviceKind::Rack; };
...
public:
   ...
   */
    ...
    {
        if (!a_simulator) throw std::runtime_error("NRE while calling Rack::SetSimulator.");

        controller_id_ = a_simulator->GetControllerID(this->cabinet());
        if (!controller_id_) return;
...
private:
   ...
   */
    aux::lsw::LoadSwitchWirings<Cabinet> lsw_wirings_{
        aux::make_wirings<Cabinet>(aux::lsw::LoadSwitchWiringFactory{}, aux::lsw::LoadSwitchChannels{})};

    aux::du::DetectorWirings<Cabinet> du_wirings_{
        aux::make_wirings<Cabinet>(aux::du::DetectorWiringFactory{}, aux::du::DetectorChannels{})};
